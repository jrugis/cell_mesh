...
def save_polylines(fname, size, pnts, slabels):
  f = open(fname, 'w')
  print map(len, pnts['label'])
  #endp = np.extract(pnts['label']=="3456", pnts)
  #endp = pnts[len(pnts['label'])==4]
  #print endp
  #print len(endp[0]['label'])
  #for t in pnts where(len(pnts['label'])==4): # find the end points
    #ijk = pnts['ijk'][t].tolist()  # get end point indices
    #slabels['hit'][tuple(ijk)] = 0 # clear end point hits
